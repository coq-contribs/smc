...
Definition var_env'_dash (ve : var_env') (n : nat) :=
  if nat_le N n then ve (n - N) else false.
...
Fixpoint be_dash (be : bool_expr) : bool_expr :=
  ...
  | Zero => Zero
  | One => One
  | Var x => Var (ad_of_nat (N + nat_of_ad x))
...
Definition renfnat (n m : nat) := if nat_le n m then m else m + N.

Definition renfnad (n : nat) (x : ad) := ad_of_nat (renfnat n (nat_of_ad x)). 
...
Proof.
  ...
  replace (nat_le N (nat_of_ad b)) with false.
  rewrite (plus_comm N (nat_of_ad b)).  reflexivity.  symmetry  in |- *.
  apply nat_le_correct_conv.  unfold lt in |- *.  apply nat_le_complete.  assumption.  
...
Proof.
  ...
  rewrite (nat_of_ad_of_nat (N + nat_of_ad b)).  apply nat_le_correct.
  apply le_plus_l.  rewrite (nat_of_ad_of_nat (N + nat_of_ad b)).
  replace (S (N + nat_of_ad b)) with (S N + nat_of_ad b).
  rewrite (plus_Snm_nSm N (nat_of_ad b)).  apply nat_le_correct.
  apply plus_le_compat.  apply le_n.  rewrite <- (plus_n_O N).
  apply nat_le_complete.  assumption.  simpl in |- *.  reflexivity.  simpl in |- *.  intros.
...
Proof.
  ...
  rewrite (nat_of_ad_of_nat (N + nat_of_ad b)).
  elim (sumbool_of_bool (nat_le N (N + nat_of_ad b))).  intro y.  rewrite y.
  rewrite (minus_plus N (nat_of_ad b)).  reflexivity.
  replace (nat_le N (N + nat_of_ad b)) with true.  intro.  discriminate.  
  symmetry  in |- *.  apply nat_le_correct.  apply le_plus_l.  intros.  simpl in |- *.
...
Proof.
  ...
  unfold augment in |- *.  elim (sumbool_of_bool (ad_eq a x)).  intro y0.  rewrite y0.
  rewrite (ad_eq_complete _ _ y0) in y.  assumption.  intro y0.  rewrite y0.
  reflexivity.  intro y.  rewrite <- H1.  unfold bool_fun_restrict in |- *.
  apply (bool_fun_of_be_ext be).  intros.  unfold augment in |- *.
  elim (sumbool_of_bool (ad_eq a x)).  intro y0.  rewrite y0.
  rewrite (ad_eq_complete _ _ y0) in y.  assumption.  intro y0.  rewrite y0.
  reflexivity.
Qed.
...
Proof.
  ...
  elim (sumbool_of_bool (nat_le n (nat_of_ad b))).  intro y.  rewrite y.
  rewrite (ad_of_nat_of_ad b).  unfold ap in |- *.
  elim (sumbool_of_bool (ad_eq (ad_of_nat n) b)).  intro y0.  rewrite y0.
  unfold renfnad, ap' in |- *.  unfold renfnat in |- *.  rewrite <- (ad_eq_complete _ _ y0).
  rewrite (nat_of_ad_of_nat n).  replace (nat_le (S n) n) with false.
  rewrite (plus_comm n N).  reflexivity.  symmetry  in |- *.  apply nat_le_correct_conv.
  auto.  intro y0.  rewrite y0.  unfold renfnad in |- *.  unfold renfnat in |- *.  rewrite y.
  rewrite (ad_of_nat_of_ad b).  replace (nat_le (S n) (nat_of_ad b)) with true.
  rewrite (ad_of_nat_of_ad b).  reflexivity.  symmetry  in |- *.
  elim (le_le_S_eq _ _ (nat_le_complete _ _ y)).  intro.  apply nat_le_correct.
  assumption.  intro.  rewrite H0 in y0.  rewrite (ad_of_nat_of_ad b) in y0.
  rewrite (ad_eq_correct b) in y0.  discriminate.  intro y.  rewrite y.
  unfold renfnad at 1 in |- *.  unfold renfnat at 1 in |- *.
  replace (nat_le (S n) (nat_of_ad b)) with false.
  replace (ad_eq (ap n) (ad_of_nat (nat_of_ad b + N))) with false.
  ...
  rewrite <- (nat_of_ad_of_nat n) in H.
  rewrite (ad_eq_complete _ _ H0) in H.
  rewrite (nat_of_ad_of_nat (nat_of_ad b + N)) in H.  apply (lt_irrefl N).
  apply le_lt_trans with (m := nat_of_ad b + N).  apply le_plus_r.
  assumption.  symmetry  in |- *.  apply not_true_is_false.  unfold not in |- *; intro.
  cut (nat_le n (nat_of_ad b) = true).  intro.  rewrite H1 in y.  discriminate.
  apply nat_le_correct.  apply le_trans with (m := S n).  apply le_S.  apply le_n.  apply nat_le_complete.  assumption.  intros.  simpl in |- *.  rewrite (H n H0).
...
Proof.
  ...
  replace
   (renamef
      (fun x : ad =>
       ad_of_nat
         match nat_le 0 (nat_of_ad x) with
         | true => nat_of_ad x
         | false => nat_of_ad x + N
         end) be) with (renamef (fun x => x) be).
  symmetry  in |- *.  apply renamef_id.  apply renamef_ext.  intro.
  rewrite (nat_le_correct 0 (nat_of_ad x) (le_O_n _)).  symmetry  in |- *.
  apply ad_of_nat_of_ad.  simpl in |- *.  intros.  rewrite (H be).  unfold replace in |- *.
...
Lemma exl_semantics :
 ...
 (forall n : nat, ve n = true -> ~ In (ad_of_nat n) lx) ->
 no_dup_list _ lx ->
...
Proof.
  ...
  unfold var_env'_to_env in |- *.  intro.  elim (ve (nat_of_ad x)); reflexivity.  
  ...
  replace (ve' (nat_of_ad x)) with false.  elim (ve (nat_of_ad x)); reflexivity.
  ...
  elim
   (H be
      (fun n : nat =>
       match ad_eq (ad_of_nat n) a with
       | true => true
       | false => ve n
       end)).
  ...
  split.  intros.  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y.
  rewrite <- (ad_eq_complete _ _ y).  left.  reflexivity.  intro y.
  ...
  unfold var_env'_to_env in |- *.  intro.  rewrite (ad_of_nat_of_ad x).
  elim (ad_eq x a).  auto with bool.  auto with bool.  unfold eval_be' in |- *.
  replace
  ...
  unfold augment, var_env'_to_env in |- *.  intro.  rewrite (ad_of_nat_of_ad x).
  rewrite (ad_eq_comm a x).  reflexivity.  intros.
  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y.
  rewrite (ad_eq_complete _ _ y).  apply no_dup_cons_no_in.  assumption.  intro y.
  ...
  unfold var_env'_to_env, var_env'_or in |- *.  intro.  rewrite (ad_of_nat_of_ad x).
  elim (sumbool_of_bool (ad_eq x a)).  intro y.  rewrite y.  simpl in |- *.
  rewrite (ad_eq_complete _ _ y).  elim (sumbool_of_bool (ve (nat_of_ad a))).
  intro y0.  elim (H0 _ y0).  rewrite (ad_of_nat_of_ad a).  left.  reflexivity.  
  ...
  unfold var_env'_to_env, augment in |- *.  intro.  rewrite (ad_of_nat_of_ad x).
  rewrite (ad_eq_comm a x).  reflexivity.  intros.
  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y.
  rewrite (ad_eq_complete _ _ y).  apply no_dup_cons_no_in.  assumption.  intro y.
  ...
  elim (sumbool_of_bool (ve' (nat_of_ad a))).  intro y.
  elim
   (H be
      (fun n : nat =>
       match ad_eq (ad_of_nat n) a with
  ...
  split.  intros.  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y0.
  rewrite y0 in H.  discriminate.  intro y0.  rewrite y0 in H.  elim (H2 _ H).
  intro.  rewrite <- H1 in y0.  rewrite (ad_eq_correct a) in y0.  discriminate.
  auto.  replace
   (eval_be' be
      (var_env'_or
         (fun n : nat =>
  ...
  unfold var_env'_to_env, var_env'_or in |- *.  rewrite (ad_of_nat_of_ad x).
  elim (sumbool_of_bool (ad_eq x a)).  intro y0.  rewrite y0.
  rewrite (ad_eq_complete _ _ y0).  rewrite y.  auto with bool.  intro y0.
  rewrite y0.  reflexivity.  unfold eval_be' in |- *.
  apply (bool_fun_of_be_ext (exl be l)).  unfold var_env'_to_env, augment in |- *.
  intros.  rewrite (ad_of_nat_of_ad x).  rewrite (ad_eq_comm a x).  reflexivity.
  intros.  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y0.
  rewrite (ad_eq_complete _ _ y0).  apply no_dup_cons_no_in.  assumption.
  ...
  unfold augment, var_env'_to_env in |- *.  intro.  elim (sumbool_of_bool (ad_eq a x)).
  intro y0.  rewrite y0.  symmetry  in |- *.  rewrite <- (ad_eq_complete _ _ y0).
  apply not_true_is_false.  unfold not in |- *; intro.  apply (H0 _ H).  left.
  symmetry  in |- *.  apply ad_of_nat_of_ad.  intro y0.  rewrite y0.  reflexivity.
  split with ve'.  split.  intros.  elim (H2 _ H).  intro.  rewrite H1 in y.
  rewrite (nat_of_ad_of_nat n) in y.  rewrite H in y.  discriminate.  auto.  
...
Proof.
  ...
  unfold var_env'_to_env in |- *.  replace (ve' (nat_of_ad x)) with false.
  elim (ve (nat_of_ad x)); reflexivity.  symmetry  in |- *.  apply not_true_is_false.
  ...
  unfold var_env'_or in |- *.  elim (ve (nat_of_ad x)); reflexivity.  simpl in |- *.
  ...
  replace
   ...
   (bool_fun_of_bool_expr (univl be l)
      (fun y : BDDvar =>
       match ad_eq a y with
       | true => true
       | false => var_env'_to_env ve y
       end)).
  assumption.  apply (bool_fun_of_be_ext (univl be l)).  intros.
  unfold var_env'_to_env in |- *.  rewrite (ad_of_nat_of_ad x).
  rewrite (ad_eq_comm a x).  reflexivity.  intros.
  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y0.  rewrite y0 in H4.
  discriminate.  intro y0.  rewrite y0 in H4.  elim (H2 _ H4).  intro.
  rewrite <- H5 in y0.  rewrite (ad_eq_correct a) in y0.  discriminate.
  trivial.  intros.  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y0.
  apply no_dup_cons_no_in.  rewrite (ad_eq_complete _ _ y0).  assumption.  
  ...
  rewrite (ad_of_nat_of_ad x).  elim (sumbool_of_bool (ad_eq x a)).  intro y0.
  rewrite y0.  simpl in |- *.  rewrite (ad_eq_complete _ _ y0).  rewrite y.
  ...
  rewrite H5 in y.  rewrite (nat_of_ad_of_nat n) in y.  rewrite y in H4.
  ...
  apply H1.  intros.  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y.
  left.  rewrite (ad_eq_complete _ _ y).  reflexivity.  intro y.
  rewrite y in H4.  right.  apply H2.  assumption.  unfold eval_be' in |- *.
  apply (bool_fun_of_be_ext be).  intros.  unfold var_env'_to_env, var_env'_or in |- *.
  rewrite (ad_of_nat_of_ad x).  elim (sumbool_of_bool (ad_eq x a)).  intro y.
  rewrite y.  auto with bool.  intro y.  rewrite y.  reflexivity.
  unfold eval_be' in |- *.  unfold var_env'_to_env in |- *.
  apply (bool_fun_of_be_ext (univl be l)).  intro.  rewrite (ad_of_nat_of_ad x).
  unfold augment in |- *.  rewrite (ad_eq_comm a x).  reflexivity.  intros.
  elim (sumbool_of_bool (ad_eq (ad_of_nat n) a)).  intro y.
  apply no_dup_cons_no_in.  rewrite (ad_eq_complete _ _ y).  assumption.  
  ...
  unfold var_env'_to_env, augment in |- *.  elim (sumbool_of_bool (ad_eq a x)).  intro y.
  rewrite y.  rewrite <- (ad_eq_complete _ _ y).  apply not_true_is_false.
  unfold not in |- *.  intro.  elim (H0 (nat_of_ad a)).  assumption.  left.
  rewrite (ad_of_nat_of_ad a).  reflexivity.  intro y.  rewrite y.  reflexivity.
...
Lemma bool_fun_of_be_ext1 :
 ...
 (forall x : ad,
  be_x_free x be = true -> ve (nat_of_ad x) = ve' (nat_of_ad x)) ->
...
Proof.
  ...
  rewrite (H b).  reflexivity.  apply ad_eq_correct.  simpl in |- *.  intros.
...
Lemma mu_all_eval_semantics1 :
 ...
 be_ok (var_lu 0 N) be ->
 forall ve : var_env',
 (forall n : nat, ve n = true -> var_lu 0 N (ad_of_nat n) = true) ->
...
Proof.
  ...
  unfold var_env'_or in |- *.  replace (ve (nat_of_ad x)) with false.  reflexivity.
  ...
  apply le_lt_trans with (m := nat_of_ad x).  apply nat_le_complete; assumption.
  unfold lt in |- *.  apply nat_le_complete; assumption.  
  apply (be_ok_be_x_free (var_lu N (2 * N)) (be_dash be)).
  apply dash_be_ok.  assumption.  assumption.  rewrite <- (ad_of_nat_of_ad x).
  ...
  unfold var_env'_dash in H5.  elim (sumbool_of_bool (nat_le N n)).  intro y.
  rewrite y in H5.  apply in_lx'.  apply nat_le_complete; assumption.  
  cut (var_lu 0 N (ad_of_nat (n - N)) = true).  unfold var_lu in |- *.  intro.
  elim (andb_prop _ _ H6).  intros.
  rewrite (nat_of_ad_of_nat (n - N)) in H8.  simpl in |- *.
  rewrite <- (plus_n_O N).  replace n with (n - N + N).
  replace (S (n - N + N)) with (S (n - N) + N).
  apply plus_le_compat.  apply nat_le_complete; assumption.  apply le_n.
  reflexivity.  rewrite (plus_comm (n - N) N).  symmetry  in |- *.
  apply le_plus_minus.  apply nat_le_complete; assumption.  apply H2.
  ...
  rewrite (nat_of_ad_of_nat n) in H9.  apply nat_le_complete; assumption.
...
Proof.
  ...
  apply andb_true_intro.  rewrite (nat_of_ad_of_nat n).  split.
  apply nat_le_correct.  apply le_O_n.  elim (in_lx'_1_conv _ _ _ (H4 _ H2)).
  intros.  apply nat_le_correct.  unfold lt in H6.
  ...
  elim (sumbool_of_bool (nat_le N (nat_of_ad x))).  intro y.  rewrite y.
  rewrite <- (le_plus_minus N (nat_of_ad x)).  reflexivity.
  apply nat_le_complete; assumption.  intro y.  rewrite y.
  replace (ve' (nat_of_ad x)) with false.  reflexivity.  symmetry  in |- *.
  apply not_true_is_false.  unfold not in |- *; intro.  unfold lx' in H4.
  elim (in_lx'_1_conv _ _ _ (H4 _ H6)).  intros.
  rewrite (nat_le_correct _ _ H7) in y; discriminate.
  ...
  replace (ve (nat_of_ad x)) with false.  simpl in |- *.
  rewrite <- (le_plus_minus N (nat_of_ad x)).  reflexivity.
  apply nat_le_complete.  assumption.  symmetry  in |- *.  apply not_true_is_false.
  unfold not in |- *; intro.  unfold var_lu in H0.  elim (andb_prop _ _ (H0 _ H10)).
  intros.  rewrite (ad_of_nat_of_ad x) in H12.  apply (lt_irrefl N).
  apply le_lt_trans with (m := nat_of_ad x).  apply nat_le_complete; assumption.    unfold lt in |- *.  apply nat_le_complete; assumption.  
  ...
  elim (andb_prop _ _ (H0 _ H2)).  rewrite (nat_of_ad_of_nat n).  intros.
  apply (lt_irrefl N).  apply le_lt_trans with (m := n).  assumption.  unfold lt in |- *.
  apply nat_le_complete.  assumption.  unfold lx' in |- *.  apply no_dup_lx'_1.
Qed.
...
Proof.
  ...
  unfold var_env'_dash in |- *.  intros.  elim (sumbool_of_bool (nat_le N n)).  intros y.
  apply in_lx'.  apply nat_le_complete.  assumption.  rewrite y in H1.
  unfold var_lu in H2.  elim (andb_prop _ _ (H2 _ H1)).
  rewrite (nat_of_ad_of_nat (n - N)).  intros.  simpl in |- *.
  rewrite <- (plus_n_O N).  replace n with (n - N + N).
  replace (S (n - N + N)) with (S (n - N) + N).
  apply plus_le_compat.  apply nat_le_complete; assumption.  apply le_n.  
  reflexivity.  rewrite (plus_comm (n - N) N).  symmetry  in |- *.
  apply le_plus_minus.  apply nat_le_complete; assumption.  intro y.
  ...
  elim (andb_prop _ _ (H0 _ H3)).  rewrite (ad_of_nat_of_ad x).  intros.
  elim (andb_prop _ _ (be_ok_be_x_free _ _ (dash_be_ok _ H) _ H1)).  intros.
  apply (lt_irrefl N).  apply le_lt_trans with (m := nat_of_ad x).
  apply nat_le_complete; assumption.  unfold lt in |- *.
  apply nat_le_complete; assumption.  unfold not in |- *; intros.  unfold lx' in H3.
  elim (in_lx'_1_conv _ _ _ H3).  intros.  unfold var_lu in H0.
  elim (andb_prop _ _ (H0 _ H1)).  rewrite (nat_of_ad_of_nat n).  intros.
  apply (lt_irrefl N).  apply le_lt_trans with (m := n).  assumption.  unfold lt in |- *.
  apply nat_le_complete.  assumption.  unfold lx' in |- *.  apply no_dup_lx'_1.
Qed.
...
Proof.
  ...
  intros.  rewrite (ad_of_nat_of_ad x) in H9.  apply (lt_irrefl N).
  apply le_lt_trans with (m := nat_of_ad x).  apply nat_le_complete; assumption.  
  unfold lt in |- *.  apply nat_le_complete; assumption.  
  apply be_ok_be_x_free with (be := be_dash be).  apply dash_be_ok.  assumption.  
  assumption.  intros.  unfold var_lu in |- *.  apply andb_true_intro.  split.
  apply nat_le_correct.  apply le_O_n.  rewrite (nat_of_ad_of_nat n).
  ...
  apply nat_le_correct.  apply (fun p n m : nat => plus_le_reg_l n m p) with (p := N).
  ...
  replace (var_env'_dash (fun n : nat => ve' (N + n)) (nat_of_ad x)) with
   (ve' (nat_of_ad x)).
  reflexivity.  unfold var_env'_dash in |- *.
  elim (sumbool_of_bool (nat_le N (nat_of_ad x))).  intro y0.  rewrite y0.
  rewrite <- (le_plus_minus N (nat_of_ad x)).  reflexivity.
  apply nat_le_complete; assumption.  intro y0.  rewrite y0.
  apply not_true_is_false.  unfold not in |- *; intro.  unfold lx' in H2.
  elim (in_lx'_1_conv _ _ _ (H2 _ H4)).  intros.
  rewrite (nat_le_correct _ _ H5) in y0; discriminate.  intro y.  rewrite y.
  ...
  rewrite (nat_of_ad_of_nat n) in H7.  apply nat_le_complete; assumption.
  unfold lx' in |- *.  apply no_dup_lx'_1.
Qed.
